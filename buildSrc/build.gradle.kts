...
buildscript {
  val kotlinVersion: String =
    if (extra.has("kotlin.version")) extra["kotlin.version"]!!.toString() else embeddedKotlinVersion
...
dependencies {
  ...
  "api"("com.gradle.publish:plugin-publish-plugin:0.10.0")

  "implementation"("com.diffplug.spotless:spotless-plugin-gradle:3.15.0")
}

val isSnapshot = extra.get("snapshot")!!.toString().toBoolean()
val pluginVersion =
  SimpleDateFormat(if (isSnapshot) "yyyyMMdd" else "yyyyMMdd-HHmmss").format(Date()) + (if (isSnapshot) "-SNAPSHOT" else "")
...
configure<GradlePluginDevelopmentExtension> {
  plugins {
    create(rootProject.name) {
      ...
      implementationClass =
        "ru.itbasis.gradle.plugin.project.kotlin.config.assistant.KotlinAssistantConfigProjectPlugin"
